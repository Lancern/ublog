...
export default function PostList({ posts }: PostListProps): JSX.Element {
  return (
    <div>
      {posts.map((post) => (
        <div className="border-b border-b-slate-200 dark:border-b-slate-800 last:border-b-0">
  ...
  return (
    <div className="py-8">
      <article>
        ...
        <div className="text-slate-400 dark:text-slate-500 my-2">
          {post.category} | {tagsString}
        </div>
        <Link href={href}>
          <button className="block dark:text-slate-200 mt-4 font-bold">
            Read more →
          </button>
        </Link>
      </article>
